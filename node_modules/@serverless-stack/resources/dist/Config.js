...
export function assertNameNotInUse(name) {
    if (Secret.hasName(name) || Parameter.hasName(name)) {
        throw new Error(`Config ${name} already exists`);
    }
}
export function codegenTypes() {
    fs.appendFileSync("node_modules/@types/serverless-stack__node/index.d.ts", `export * from "./config";`);
    fs.writeFileSync("node_modules/@types/serverless-stack__node/config.d.ts", `
